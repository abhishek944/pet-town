...
scopes = sorted(
    set(posixpath.normpath(p.replace("\\", "/").strip() or ".") for p in (args.scope or ["."]))
)
...
update_process(
    digest, ["git", "-C", str(root), "diff", "--binary", "--no-ext-diff", base, *pathspec]
)
...
result = {
    ...
    "target_sha": None,
    "scope": scopes,
    "streamedBinaryDiff": True,
    "exactBinaryPaths": sorted(binary_scopes.intersection(scopes)),
}
