...
import { CAPABILITIES, type CapabilityId } from "@pet-village/core";
import { ACTIVE_PET_ID, installCapabilityMapping, PETS, type CapabilityMapping } from "./pets";
...
export class StudioSettings {
  ...
  markup(): string {
    ...
    return `<header><h1>Capabilities</h1><p>Map reviewed sprite sheets to trusted engine behaviors.</p></header><div class="studio-layout"><section class="studio-preview"><div class="studio-sprite" role="img" aria-label="Plum Dragon ${CAPABILITIES[this.#selected].label} animation preview"></div><small>Preview: Plum Dragon · ${CAPABILITIES[this.#selected].label}</small></section><section class="capabilities">${capabilities
      .map((id) => {
        const ready = Boolean(PETS[ACTIVE_PET_ID].assets[id]);
  ...
  bind(content: HTMLElement): void {
    ...
    content.querySelectorAll<HTMLButtonElement>("[data-capability]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const needsMapping = !PETS[ACTIVE_PET_ID].assets[this.#selected];
        this.rerender();
    ...
    fileInput?.addEventListener("change", () => {
      ...
      if (file)
        void this.#import(file, frames, capability, viewGeneration).catch((error: unknown) => {
  ...
  async #testPreview(content: HTMLElement): Promise<void> {
    const capability = this.#selected;
    const asset = PETS[ACTIVE_PET_ID].assets[capability];
  ...
  async #import(
    ...
    if (!installCapabilityMapping(mapping))
      throw new Error(`${CAPABILITIES[capability].label} is not enabled for Plum Dragon.`);
  ...
  #configurePreview(content: HTMLElement): void {
    const preview = content.querySelector<HTMLElement>(".studio-sprite");
    const asset = PETS[ACTIVE_PET_ID].assets[this.#selected];
