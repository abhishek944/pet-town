...
fn install_tray(app: &tauri::App) -> tauri::Result<()> {
    ...
    let quit = MenuItem::with_id(app, "quit", "Quit Pet Village v2", true, None::<&str>)?;
    let menu = Menu::with_items(app, &[&settings, &playroom, &quit])?;
    let mut tray = TrayIconBuilder::new().menu(&menu).tooltip("Pet Village v2");
...
pub fn run() {
    tauri::Builder::default()
        ...
        .expect("failed to run Pet Village v2");
}
