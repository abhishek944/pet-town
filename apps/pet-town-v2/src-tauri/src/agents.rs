...
fn pet_for(_id: &str) -> String {
    // The frontend assigns display pets by stable hash; the bridge leaves this unassigned.
    String::new()
}

fn bridge_binary() -> Option<PathBuf> {
    if let Some(binary) = std::env::var_os("PET_TOWN_V1_BIN") {
        return Some(PathBuf::from(binary));
    }
    let mut candidates = Vec::new();
    if let Ok(executable) = std::env::current_exe() {
        if let Some(directory) = executable.parent() {
            candidates.push(directory.join("pet-town"));
        }
    }
    ...
    let v1_target = apps.join("pet-town/src-tauri/target");
    candidates.push(v1_target.join(target).join("release/pet-town"));
    candidates.push(v1_target.join("debug/pet-town"));
