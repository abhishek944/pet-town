...
pub fn relay(source: &str, event: &str, install_epoch: Option<&str>) -> Result<(), String> {
    ...
    let generation =
        std::env::var("PET_VILLAGE_HOOK_GENERATION").unwrap_or_else(|_| super::opaque_key(session));
    ...
    command
        ...
        .env("PET_VILLAGE_HOOK_GENERATION", generation);
    if let Some(epoch) = epoch {
        command.env("PET_VILLAGE_ADAPTER_EPOCH", epoch);
    }
