...
export function compilePackActions(
  ...
  if (names.length > ACTIONS_MAX) {
    diagnostic(
      ...
      "E_ACTION_LIMIT",
      "/actions",
      `A pack may declare at most ${ACTIONS_MAX} actions`,
    );
  }
  for (const name of names.slice(0, ACTIONS_MAX)) {
    ...
    if (!IDENTIFIER_PATTERN.test(name) || !isRecord(value)) {
      diagnostic(
        ...
        "E_ACTION",
        path,
        "Action must have a lowercase identifier and object definition",
      );
      continue;
    }
    ...
    if (
      ...
      diagnostic(
        ...
        "E_ACTION_LABEL",
        `${path}/label`,
        "Action label must be 1-24 trimmed characters",
      );
    }
    ...
    if (result.minimumMs <= 0) {
      diagnostic(
        ...
        "E_ZERO_TIME_CYCLE",
        `${path}/flow`,
        "Every action must consume logical time",
      );
    }
    if (result.maximumMs > LIMITS.restartMaxMs) {
      diagnostic(
        ...
        "E_ACTION_DURATION",
        `${path}/flow`,
        "Actions must finish within the supported duration",
      );
    }
    if (
      result.node &&
