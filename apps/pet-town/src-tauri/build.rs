...
fn compile_wake_bridge() {
    ...
    let library = output.join("libpet_town_wake.a");
    ...
    println!("cargo:rustc-link-lib=static=pet_town_wake");
...
fn main() {
    ...
    println!(
        "cargo:rustc-env=PET_TOWN_PI_RUNTIME_SHA256={}",
        runtime_hash()
    );
    println!(
        "cargo:rustc-env=PET_TOWN_PET_IDS={}",
