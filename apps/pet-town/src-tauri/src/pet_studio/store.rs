...
pub fn root() -> Result<PathBuf, String> {
    ...
    Ok(PathBuf::from(home).join(".pet-town").join("pet-packs"))
}
...
fn reject_private_symlinks(path: &Path) -> Result<(), String> {
    for candidate in path.ancestors() {
        ...
        if candidate.file_name().and_then(|name| name.to_str()) == Some(".pet-town") {
            break;
        }
    }
    Ok(())
...
pub fn create_private_dir(path: &Path) -> Result<(), String> {
    ...
    for directory in path.ancestors() {
        ...
        if directory.file_name().and_then(|name| name.to_str()) == Some(".pet-town") {
            break;
        }
    }
    Ok(())
