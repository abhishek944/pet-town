...
pub fn record(source: &str, event: &str, install_epoch: Option<&str>) -> Result<(), String> {
    ...
    let generation = std::env::var("PET_TOWN_HOOK_GENERATION")
        .ok()
        .or_else(|| {
    ...
    let received_epoch = install_epoch
        .map(str::to_string)
        .or_else(|| std::env::var("PET_TOWN_ADAPTER_EPOCH").ok());
...
fn publish(
    ...
    if let Some(expected) = super::enabled::epoch(source) {
        match install_epoch {
            ...
            None if std::env::var("PET_TOWN_ALLOW_UNVERSIONED").ok().as_deref() == Some("1") => {}
            _ => return Ok(()),
        }
    }
