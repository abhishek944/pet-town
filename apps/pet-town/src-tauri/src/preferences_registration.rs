...
impl PreferencesStore {
    ...
    pub fn register_pet(&self, id: String, _assign_orchestrator: bool) -> PreferencesSnapshot {
        let mut state = self.state.lock().unwrap_or_else(|error| error.into_inner());
        ...
        if !state.read_only {
            if let Some(path) = &self.path {
                ...
                if let Err(error) = crate::preferences_lock::exclusive(&path, || {
                    merge_and_save(&path, &mut state, &id)
                }) {
...
fn reconcile(state: &mut StoreState) {
    ...
    state.applied.normalize_assistant();
...
fn merge_and_save(path: &std::path::Path, state: &mut StoreState, id: &str) -> Result<(), String> {
