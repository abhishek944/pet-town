...
pub async fn ensure_default(app: AppHandle) -> Result<(), String> {
    let current = app
        ...
        .clone();
    let workspaces = tauri::async_runtime::spawn_blocking(super::herdr::workspaces)
        .await
        .map_err(|_| "Could not inspect Herdr workspaces.".to_string())??;
    let workspace = current
        .as_ref()
        .and_then(|id| workspaces.iter().find(|item| &item.id == id))
        .or_else(|| workspaces.first())
        .ok_or_else(|| "No Herdr workspace is available.".to_string())?;
    ensure(app, workspace.id.clone()).await
}
...
pub async fn ensure(app: AppHandle, workspace_id: String) -> Result<(), String> {
    ...
    let (existing, generation) = {
        ...
        if runtime.exiting {
            return Err("Pet Village is waiting to close safely.".into());
        }
