...
pub fn test(app: &AppHandle, name: &str) -> Result<(), String> {
    begin(app, name, true, false)
}

fn begin(app: &AppHandle, name: &str, testing: bool, automatic: bool) -> Result<(), String> {
    if !supported() {
        return Err("On-device wake recognition is unavailable; use push-to-talk.".into());
    }
    let phrase = CString::new(format!("hey {}", name.trim().to_lowercase()))
        .map_err(|_| "The orchestrator name cannot be used as a wake phrase.".to_string())?;
...
extern "C" fn wake_callback(found: bool, message: *const c_char, generation: u64) {
    ...
    if found && testing {
        let _ = app.emit("orchestrator-wake-status", "Wake phrase test passed.");
        restart(app);
    } else if found {
        let app = app.clone();
