...
pub(super) fn verify_from_environment(pane_id: &str) -> Result<(), String> {
    let expected = std::env::var("PET_VILLAGE_EXPECTED_AGENT_SESSION")
        .map_err(|_| "expected agent session is unavailable".to_string())?;
    let socket = std::env::var("HERDR_SOCKET_PATH").ok();
    let binary = std::env::var_os("PET_VILLAGE_HERDR_BIN")
...
pub(super) fn focus(
    ...
    focus_verified(herdr, pane_id, socket, &verified)
}
