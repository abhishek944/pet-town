...
if args.remove_light_background:
    ...
    while queue:
        ...
        if min(red, green, blue) < 210 or max(red, green, blue) - min(red, green, blue) > 18:
            continue
        background.add((x, y))
        queue.extend((nx, ny) for nx, ny in ((x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1))
...
def reduce_spans(groups, expected):
    ...
    return [(min(item[0] for item in cluster), max(item[1] for item in cluster)) for cluster in assigned]
...
cells = [
    sheet.crop((column * cell_width, row * cell_height,
...
for cell in cells:
    ...
    frame.alpha_composite(figure, ((args.width - figure.width) // 2, args.height - figure.height - 4))
    frames.append(frame)
font_paths = [Path("/System/Library/Fonts/Supplemental/Arial Black.ttf"),
...
def electric_z(canvas, position, size, opacity):
    ...
    ImageDraw.Draw(mask).text(position, "Z", font=font, fill=opacity, stroke_width=5,
                              stroke_fill=opacity, anchor="lt")
    ...
    draw.text(position, "Z", font=font, fill=(248, 253, 255, opacity), stroke_width=8,
              stroke_fill=(3, 18, 31, opacity), anchor="lt")
    draw.text(position, "Z", font=font, fill=(248, 253, 255, opacity), stroke_width=4,
              stroke_fill=(41, 184, 242, opacity), anchor="lt")
...
if args.sleep_z_trail:
    ...
    for frame_index, frame in enumerate(frames):
        ...
        for z_index, (position, size) in enumerate(zip(positions, sizes)):
...
frames[0].save(output, save_all=True, append_images=frames[1:], format="PNG",
               duration=durations, loop=0, disposal=1, blend=0, optimize=False)
if args.contact:
    ...
    for index, frame in enumerate(frames):
        ...
        ImageDraw.Draw(contact).text(((index % 4) * frame_width + 5,
                                     (index // 4) * frame_height + 5), str(index + 1), fill="white")
