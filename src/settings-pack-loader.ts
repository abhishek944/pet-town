...
import { installUserPacks, type PetExtensionPayload, type UserPackPayload } from "./character-packs";
...
export async function loadPetPacks(): Promise<string | null> {
  ...
  try {
    ...
    const warnings = [...catalog.warnings, ...rejected.map((id) => `${id}: incompatible with its base pet`)];
